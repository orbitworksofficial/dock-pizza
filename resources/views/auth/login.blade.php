...
@section('title', 'Login — Pizza Viva')
...
<div class="py-24 max-w-md mx-auto px-4 sm:px-6">
    <div class="bg-white border border-stone-200 rounded-3xl p-8 space-y-6 shadow-sm" data-aos="zoom-in" data-aos-duration="500">
        <div class="text-center space-y-2">
            <h1 class="text-3xl font-extrabold text-[#1E1E1E] font-serif">Welcome Back</h1>
            <p class="text-stone-500 text-sm">Sign in to track orders, save favorites, and earn loyalty points.</p>
        </div>
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-4">
            ...
            <div>
                <div class="flex items-center justify-between mb-2">
                    <label for="password" class="block text-xs font-bold uppercase tracking-wider text-stone-600">Password</label>
                    <a href="#" class="text-xs text-[#F37021] hover:text-[#FDB813] transition-colors font-semibold">Forgot Password?</a>
                </div>
                <input type="password" name="password" id="password" required class="w-full bg-[#F3F4F6] border border-transparent rounded-2xl py-3 px-4 text-[#1E1E1E] focus:outline-none focus:bg-white focus:border-[#FDB813] text-sm transition-all placeholder-stone-400" placeholder="Enter your password">
            </div>

            <div class="flex items-center">
                <input type="checkbox" name="remember" id="remember" class="rounded border-stone-300 text-[#FDB813] focus:ring-[#FDB813]">
        ...
        <div class="border-t border-stone-100 pt-6 text-center text-stone-500 text-sm">
            ...
            <a href="{{ route('register') }}" class="text-[#F37021] hover:text-[#FDB813] font-semibold transition-colors">Register</a>
        </div>
    </div>
</div>
@endsection
